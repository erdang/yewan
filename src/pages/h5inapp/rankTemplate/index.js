...
const MoneyRank = ({
    ...
    let toFunction = useCallback((event) => {
        ...
        let Uid = event.currentTarget.dataset.uuid;
        if (appGate.inApp()) {
            if (searchParam.room !== 'full') {
                console.log(1);
                if (rid !== '0') {
                    appGate.openRoom(Uid, rid);
                } else {
    ...
    if (themeStyle === 2) {
        ...
        if (
            ...
            rankTop = null;
            rankTopStyle = { ...headBg, marginTop: 0, color: '#fff' };
        } else {
            rankTop = (
                <pre
                    className="text-rule"
                            ...
                            return (
                                <li key={index}>
                                    ...
                                    >
                                        ...
                                        <div
                                            className="info"
                                            data-rid={item.onRoom.rid}
                                            data-uuid={item.onRoom.uid}
                                            data-uid={item.uid}
                                            onClick={toFunction}
                                        >
                                            <div className="pic">
...
const CommonPaste = ({ page, pageID }) => {
    ...
    let [info] = useState(() => {
        return JSON.parse(searchParam.event_data);
    });
    let clickFn = useCallback(() => {
        let url =

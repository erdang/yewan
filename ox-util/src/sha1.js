...
export default class SHA1 {
    static hash(string) {
        return SHA1.stringToHex(
            SHA1.arrayToString(
                SHA1.run(SHA1.stringToArray(string), string.length * 8),
            ),
        );
    }
    ...
    static run(input, len) {
        ...
        for (; i < l; i += 16) {
            ...
            for (; j < 80; j += 1) {
                ...
                t = SHA1.add(
                    SHA1.add(SHA1.rotl(a, 5), SHA1.chMajPty(j, b, c, d)),
                    SHA1.add(SHA1.add(e, W[j]), SHA1.cnst(j)),
                );
                e = d;
                d = c;
                c = SHA1.rotl(b, 30);
